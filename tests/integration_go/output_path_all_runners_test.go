...
func TestConsoleOutputPath_Vitest(t *testing.T) {
    ...
    if err := exec.Command("npx", "vitest", "--version").Run(); err != nil { t.Skipf("vitest not available: %v", err) }
...
func TestConsoleOutputPath_Pytest(t *testing.T) {
    if _, err := exec.LookPath("pytest"); err != nil { t.Skip("pytest not found in PATH") }
    ...
    if len(trunMatch) >= 2 && len(seeMatch) >= 3 {
        ...
        reportPath := filepath.Join(fixtureDir, seePath)
        if _, err := os.Stat(reportPath); err == nil { return }
    }
