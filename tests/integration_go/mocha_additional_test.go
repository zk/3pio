...
func TestMochaPatternMatching(t *testing.T) {
    ...
    if err := testutil.CommandAvailable("npx", "mocha", "--version"); err != nil { t.Skipf("mocha command failed: %v", err) }
    ...
    if result.ExitCode != 0 { t.Fatalf("expected success running pattern, got %d", result.ExitCode) }
    ...
    if len(entries) < 1 { t.Errorf("expected at least 1 report dir for pattern run, got %d", len(entries)) }
}
...
func TestMochaMissingFileHandling(t *testing.T) {
    ...
    if result.ExitCode == 0 { t.Fatalf("expected non-zero exit for missing file") }
    runsDir := filepath.Join(fixtureDir, ".3pio", "runs")
    if _, err := os.Stat(runsDir); os.IsNotExist(err) { t.Error("expected .3pio/runs to exist even on error") }
}
