...
func TestCypressFullRun_AllSpecs_Fails(t *testing.T) {
	...
	if !strings.Contains(string(data), "detected_runner: cypress") {
		t.Error("expected detected_runner: cypress")
	}
	if !strings.Contains(string(data), "FAIL") {
		t.Error("expected failure recorded in report")
	}
	...
	if len(entries) < 2 {
		t.Errorf("expected at least 2 report dirs, got %d", len(entries))
	}
}
...
func TestCypressPatternMatching(t *testing.T) {
	...
	if len(entries) == 0 {
		t.Fatal("expected at least one report dir for pattern run")
	}
}
...
func TestCypressMissingSpecHandling(t *testing.T) {
	...
	if result.ExitCode == 0 {
		t.Fatalf("expected non-zero exit for missing spec")
	}
